...
PFN_vkVoidFunction layer_vkGetInstanceProcAddr(
    VkInstance instance,
    const char* pName) {
...
extern "C" __attribute__((visibility("default")))
PFN_vkVoidFunction layer_vkGetDeviceProcAddr(
    VkDevice device,
    const char* pName) {
    const std::string name(pName);
    auto it = layerFunctions.find(name);
